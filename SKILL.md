...
**Status:** **v2.17.0 — STABLE.** Bilingual AI-text humanizer (Arabic primary, English secondary). Hard dependency on arabic-corpus-toolkit (G1 normalize / G2 registry / G3 telemetry / G4 install adopted at runtime; `safe_llm_call` adopted in `score_text_deep` as of v2.17.0). Input validation guards `None` / non-string / oversized inputs with structured envelopes; `score_text_deep` routes through `safe_llm_call` (retries + per-vendor circuit breaker). **On the 16-vs-13 dimension count:** the framework defines **16 dimensions**; the deep LLM cognitive diagnostic (`analyze_deep.py`) scores **dimensions 1–13** (cognitive + rhetorical + coherence), while **dimensions 14–16** (reader-respect, typography hygiene, connector entropy) are measured by the deterministic lex/typography pass rather than the LLM — so “13-dim diagnostic” and “16-dim framework” name two layers, not a contradiction. **Full version history → [`CHANGELOG.md`](CHANGELOG.md).**
