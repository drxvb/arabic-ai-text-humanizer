...
**Status:** **v2.17.0 — STABLE + input validation + LLM failure resilience (3-of-4 A7 convergent must-have).** Two-part shipment closing the 3-of-4 A7 vendor must-have (codex + minimax + deepseek all flagged "no production-grade error handling; None crashes it"). (1) `score_text()` now runs `_validate_input_text()` BEFORE any processing — returns structured `{ok: False, input_validation: {error_class, error_detail}}` for `None` (`null_input`), non-string (`wrong_type`), and pathologically-long inputs (`input_too_long`, MAX_INPUT_CHARS_HEURISTIC=200K). Smoke verified: `score_text(None)`, `score_text(12345)`, `score_text("x"*250000)` all return structured envelopes instead of crashing. (2) `score_text_deep()` now routes through toolkit v1.13.0's `safe_llm_call` for retries + circuit breaker + structured `llm_error_class`. Heuristic fallback path enriched with `error_class` + `circuit_open` + `attempts` metadata so consumers can distinguish transient vs. terminal LLM failures. Closes minimax's verbatim verdict: "the single biggest blocker is that this skill has NO production-grade error handling." **v2.16.1 — STABLE + English G1-equivalent regression suite (2-of-2 A6 convergent gap).** `evals/test_english_normalize_regression.py` — 16 targeted assertions codifying the English-side normalize-before-pattern-match contract: case-insensitive scan (all-caps + title + lower + mixed produce identical findings), whitespace tolerance (extra spaces + tabs + newlines after comma), case-preserving substitution (`Navigate→Handle`, `NAVIGATE→HANDLE`, lowercase replacement convention), KILLER mutation-resistance fixture (heavily-mutated input `"LOOK,    this  is  INcREDIbly important..."` produces SAME findings count + SAME score total as canonical original). Documents the design choice that period/semicolon variants are NOT normalized to comma-form (treated as distinct rhetorical moves by the catalogue). 16/16 PASS. Closes the 2-of-2 A6 multi-vendor convergent gap (minimax-M2 + deepseek-v4-pro both flagged: "English humanization path lacks equivalent G1 regression suite — Arabic has 16 regression fixtures but English has no documented normalize-before-pattern-match gates"). The English-side guard mirrors v2.16.0's Arabic G1 killer fixture — together they're the load-bearing regression gates against contributors silently breaking normalization in either language. **v2.16.0 — STABLE + G1 normalize-before-AI-tell regression suite.** Closes Codex A3 carry-over (deferred 3 releases) + Kimi A5 #3 leverage action: "G1 normalize-before-AI-tell-count regression fixtures still not added". `evals/test_g1_normalize_regression.py` — 16 targeted before/after assertions across 5 fixtures: A (tashkeel vs. plain — same AI-tell hit count required), B (tatweel-stripped vs. plain), C (light-level preserves alif variants vs. medium-level over-collapse), D (empty/clean edge cases), E (KILLER mutation-resistance: heavily-tashkeel-saturated input MUST still match the dictionary or G1 contract was bypassed). 16/16 PASS — the killer fixture proves that `score_text("مًنٌ اَلُمِهّمْ مٌلٍاَحُظِةّ")` still finds "من المهم ملاحظة" because `_arabic_normalize_via_toolkit` fires before lookup. The single most important regression gate against future contributors silently removing the G1 normalization call from `score_text`. **v2.15.1 — STABLE.** Closes Codex+Gemini A4 #2 finding: "humanizer SKILL.md lacks a clear top-level current-status banner; versions are buried in a large table." v2.15.1 surfaces current state at-a-glance. The toolkit is a hard dependency (v2.8.0+); all four foundational contracts (arabic_normalize G1 / asset_registry G2 / influence_telemetry G3 / install_family G4) are adopted at runtime call sites. `score_text(emit_trace=True)` returns Asset C influence records per AI-tell match; `score_text_deep(emit_trace=True)` returns humanizer_gate_decision records on every code path with `fallback_used: bool` distinguishing LLM-backed scores from heuristic-fallback. Vendor rotation across {gemini, minimax, codex} for `score_text_deep` per call. `score_text_multivendor` returns cross-LLM agreement bands. Asset D + E consumer cutover complete (typography normalization + reader-respect anti-pattern detection). 107/107 regression green. Version history table below.
...
|---|---|
| **v2.16.0** | **G1 normalize-before-AI-tell-count regression suite** (Codex A3 carry-over deferred 3+ releases + Kimi A5 #3). `evals/test_g1_normalize_regression.py` — 16 targeted before/after assertions across 5 fixtures: (A) tashkeel vs. plain — same AI-tell hit count required when G1 fires; (B) tatweel-stripped vs. plain; (C) light-level preserves alif variants (catches over-collapse if anyone wrongly upgrades to medium); (D) empty / clean MSA edge cases; (E) KILLER mutation-resistance — `score_text("مًنٌ اَلُمِهّمْ مٌلٍاَحُظِةّ")` MUST still return 1 hit because `_arabic_normalize_via_toolkit` fires before dictionary lookup. The killer fixture is the single most important regression gate against future contributors silently removing the G1 normalize call from `score_text`. 16/16 PASS. The last individual leverage action from any A5 evaluator (codex + kimi both listed it). |
| **v2.15.1** | **Documentation reconciliation: SKILL.md status banner.** Closes Codex+Gemini A4 #2 finding: "humanizer SKILL.md lacks a clear top-level current-status banner; versions are buried in a large table." v2.15.1 surfaces current state at-a-glance with explicit contract-adoption matrix (G1+G2+G3+G4 all marked live at runtime call sites). No code change. |
| **v2.15.0** | **Closes Sonnet third-audit findings: registry runtime adoption + score_text_deep telemetry + fallback_used flag.** Sonnet flagged that `_registry_is_compatible` had **zero call sites** (defined but never used — "scaffolding, not adoption"). v2.15.0 promotes the helper to module-init scope (defined immediately after `_TOOLKIT_DEFAULT_PATH`) so the lexical-tables loader at line 1334 actually calls `_registry_is_compatible("C", observed)` instead of the inline `schema_major == "1"` check. Verified: `AI_PHRASES_AR loaded: 67 entries` (loader ran via registry). Also adds `emit_trace=True` kwarg to `score_text_deep` (previously only `score_text` had it — Sonnet flagged the "premium path is still a telemetry black box"). Also adds `fallback_used: bool` field to every `score_text_deep` return path so consumers can distinguish LLM-backed scores from heuristic-fallback scores (Codex's third-audit "silent quality degradation" risk + documented v2.10.0 TODO). 74/74 Arabic regression green. |
| **v2.14.0** | **Cross-contract adoption (Gaps G1+G2+G3 in one shot)** — re-audit consensus #1. Sonnet/Codex/Gemini all flagged humanizer as having adopted 0 of 4 toolkit contracts. v2.14.0 closes that: `_arabic_normalize_via_toolkit()` routes through toolkit v1.5.0's `arabic_normalize.normalize()` (with raw-text fallback); `_registry_is_compatible()` routes through toolkit v1.6.0's `asset_registry.is_compatible()` (with legacy major-version fallback); `_new_humanizer_trace()` instantiates toolkit v1.7.0's `InfluenceTrace` (returns None when toolkit absent). `score_text()` now: (a) normalizes input via the shared contract before counting AI-tells so `الذكاءِ` and `الذكاء` collide identically; (b) accepts `emit_trace=True` and returns `influence_trace: [...]` with one Asset C record per AI-phrase substring match. Verified: sloppy text scored 0/100 with 3 trace records, each citing Asset C v1.1.0 + phrase + count. 74/74 Arabic regression green. The module-init loader (line 1334) keeps legacy `schema_major == "1"` check because helpers are defined later in source order — documented inline. |
| **v2.13.1** | **Documentation reconciliation** in response to the 3-evaluator audit (Sonnet found this SKILL.md ended at v2.9.0 while code implemented v2.10–v2.13). No code change; backfills version-history entries for v2.10.0 / v2.11.0 / v2.12.0 / v2.13.0 (all listed below). |
| **v2.13.0** | **Vendor rotation in `score_text_deep` (roadmap item #5).** Multi-agent roadmap consensus (minimax #1, codex #5). `score_text_deep` no longer pins to gemini per call — rotates across `{gemini, minimax, codex}` per invocation. Consumers wanting a specific vendor pass `proxy_name=...`; rotation fires when `proxy_name is None`. Verified: same text scored gemini=100, minimax=80, codex=80, gemini=100 across 4 successive rotation calls — vendor variance now visible to consumers instead of hidden by single-vendor lock-in. |
| **v2.12.0** | **Asset D + E consumer cutover (roadmap item #1).** Multi-agent roadmap consensus item #1: half the toolkit's data assets were dormant (no consumer reading them). v2.12.0 imports both. Asset D (`typography-rules.json` v1.0.0, 9 rules) → `apply_typography_rules()` converts ASCII punctuation surrounded by Arabic letters to Arabic punctuation, strips space before Arabic punctuation. Asset E (`reader-respect-patterns.json` v1.0.0, 6 anti-patterns) → `reader_respect_score()` detects tautology / re-explanation / forced-conclusion markers. Verified: sloppy text caught 4 anti-patterns across 3 categories at density 250/1k. Regression: 74 AR + 33 EN = 107/107 green. |
| **v2.11.0** | **`score_text_multivendor()` for cross-LLM scoring agreement.** Applies the cross-vendor pattern (toolkit v0.9.1/v0.10) to humanness scoring. Calls `score_text_deep` across multiple proxies (default gemini + minimax), reports mean / range / std_dev / agreement band (strong if range≤10, moderate if ≤25, weak otherwise). Verified: clean text scored gemini=92 / minimax=85, range=7 → strong agreement; per-dimension showed both agreed on register_match=25 but diverged on factual_anchor (gemini 20, minimax 15) — exactly the kind of signal cross-vendor surfaces. |
| **v2.10.0** | **`score_text_deep()` LLM-backed cognitive scoring via proxy fleet.** v2.9.0 shipped the heuristic. v2.10.0 adds `score_text_deep` with a 4-dimension cognitive rubric (directness, naturalness, register_match, factual_anchor) each scored 0-25 by an LLM proxy (default at the time was gemini-2.5-flash; vendor rotation added in v2.13.0). Verified: clean tech-news text scored 94/100 (24+25+25+22 across dimensions) with per-dimension reasoning; sloppy AI-tell-saturated text scored 10/100 (5+5+0+0) with reasoning citing specific hedge phrases by name. Heuristic remains the fast default; deep is for borderline cases. Falls back to heuristic on LLM error. **Known silent-fallback risk (flagged by v2.13.1 evaluator audit):** the heuristic-fallback path returns the same response shape as a real LLM call; consumers can't tell whether the score came from a real LLM or the fallback. A future v2.14+ may add an explicit `fallback_used: bool` field to surface this. |
| **v2.9.0** | **`score_text(text_ar, register)` exposed as importable function for consumer use.** Replaces the authoring-suite's hard-coded 6-tell heuristic with the full Asset C coverage (67 AI-phrases in v1.1.0 + 8 intensifier-destack regex patterns). Returns `{score, ai_tell_hits, ai_tell_density_per_1k, total_words, ai_phrases_caught, sample_size}`. Density formula: `max(0, 100 - density_per_1k * 5)` — empirically calibrated so clean text → 100, one tell per 50 words → ~75, AI-saturated → 0. Importable via `from humanize_v2 import score_text`. Verified: clean text scores 100/100 with 0 hits; 6-tell sloppy text scores 0/100 with all 6 phrases identified by name. 107/107 regression green. Consumers (translator Stage E, authoring-suite humanizer_gate) can now use real corpus-grounded scoring instead of heuristics. |
| **v2.8.0** | **Vendored lexical literals REMOVED. Toolkit is now a hard dependency.** Completes the migration trajectory committed in v2.7.0 → v2.7.1 → v2.7.2. The six lexical tables (AI_PHRASES_AR, CONNECTORS_AR, REPETITIVE_STARTERS_AR, STRUCTURAL_OPENERS_AR, QUOTE_VERBS_ROTATION, INTENSIFIER_DESTACK) are now empty stubs at module load and populated from `arabic-corpus-toolkit/corpus/lexical-tables.json`. If the toolkit is unavailable, module load raises ImportError with explicit install guidance (unless `ARABIC_CORPUS_TOOLKIT_DISABLE=1` is set for isolation testing). **Eval evidence: 74/74 Arabic + 33/33 English regression green** with toolkit backing. DISABLE=1 path confirmed (empty stubs, 0 entries). Historical inline literals retained inert as `_REMOVED_INLINE_*_AT_V2_8_0` references for git-blame archaeology only. This is the trajectory v2.7.x committed to and v2.8.0 executes — toolkit becomes single source of truth for lexical substitution policy across all three sibling consumers (humanizer, translator, authoring-suite). |
